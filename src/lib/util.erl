...
%%从N个数的集合中随机出M个互不相同的数，每个数几率均等
rand_m_in_n(L, M) when is_list(L) ->
    N = length(L),
    case M =:= N of
        true -> L;
        false ->
            case M > N of
                true -> [];
                false -> rand_m_in_n(L, N, [], N-M+1)
            end
    end.

rand_m_in_n(_, N, Acc, I) when I > N ->
    Acc;
rand_m_in_n(L, N, Acc, I) ->
    T = lists:nth(rand(1, I), L),
    case lists:member(T, Acc) of
        false -> rand_m_in_n(L, N, [T|Acc], I+1);
        true ->  rand_m_in_n(L, N, [lists:nth(I, L)|Acc], I+1)
    end.
